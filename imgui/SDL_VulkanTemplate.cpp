...
int main(int, char**)
{
	...
	//Fonts
	io.Fonts->AddFontFromFileTTF("SourceHanSansSC-Normal.otf",
		28.0f, NULL, io.Fonts->GetGlyphRangesChineseFull());  // Load Japanese characters
	...
	while (!done)
	{
		...
		// 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
		{
			...
			bool p = false;
			ImGui::Begin("Hello",&p, ImGuiWindowFlags_NoTitleBar);
		
			//ImGui::SetNextWindowFocus();
			ImVec2 base_pos = ImGui::GetCursorScreenPos();
			cout << base_pos.x << " " << base_pos.y << endl;
			//ImGui::BeginChild("Hello, world!");                          // Create a window called "Hello, world!" and append into it.
			
			auto dl = ImGui::GetWindowDrawList();
			dl->AddCircle(ImVec2(base_pos.x+100, base_pos.y+100), 100,ImColor(255,0,0), 30,3);
		
			ImGui::TextWrapped("辛几何（英语：Symplectic geometry），也叫辛拓扑（英语：Symplectic topology），是微分几何的一个分支。其研究对象为辛流形，亦即带有闭非退化2-形式的微分流形。辛拓扑源于经典力学的哈密顿表述，其中特定经典系统的相空间有辛流形的结构。", 123);
